...
const DashboardBarChart = ({ data }: Props) => {
  ...
  return (
    <BarChart<TSalesBreakDownResponse>
      xKey={'name'}
      xTickFormatter={(value) => {
        const totalPrice =
          data?.find((d) => d.name === `${value}`)?.totalPrice ?? 0;

        return `(${totalPrice.toLocaleString()})`;
      }}
      data={data}
      chargAvg={chargAvg}
      tooltipFormatter={(_, name, row) => {
        const value =
          name === '매출액' ? (row?.totalPrice ?? 0) : (row?.profitPrice ?? 0);
        return [`${value.toLocaleString()}`, name];
      }}
      series={[
        {
          key: 'profitPriceForChart',
    ...
    //             tick={({ x, y, payload }) => {
    //               const totalPrice =
    //                 data?.find((d) => d.name === payload.value)?.totalPrice ??
    //                 0;
